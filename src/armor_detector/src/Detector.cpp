...
//        auto greenDetector = new YOLOv10Detector("./model/0715_v10_416n_openvino_model/0715_v10_416n.xml", 0.20);
        ...
        TimeSystem::start_time = std::chrono::steady_clock::now();
        ...
        while (!(*_detector_thread_params.frame_pp)->mat->empty())
        {
            ...
            // 相机时间戳以及imu的时间戳
            // 确认一下时间戳是否同步
            // DLOG(INFO) << " cam time_stamp: " << std::chrono::duration_cast<std::chrono::microseconds>((image.time_stamp) - TimeSystem::start_time).count() / 1000.0
...
///////////////////////////////////////////////////////////////////////////////////////////////
            ...
            // 写入数据的时间戳
            SerialParam::send_data.time_stamp = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - TimeSystem::start_time).count() / 10.0;
